...
Feature: Check that settings are adhered to when creating an enrolment plugin
  In order to create an poodllprovider enrolment instance
  As an admin
  ...
  Background:
    ...
    And I navigate to "Plugins > Enrolments > Publish as poodllprovider tool" in site administration
  ...
  Scenario: As an admin set site-wide settings for the enrolment plugin and ensure they are used
    ...
    And I click on "Edit" "link" in the "Publish as poodllprovider tool" "table_row"
