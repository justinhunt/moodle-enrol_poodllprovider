...
/**
 * Displays enrolment poodllprovider instances.
 *
...
/**
 * Handles displaying enrolment poodllprovider instances.
 *
 * @package    enrol_poodllprovider
 * @copyright  2020 Justin Hunt <justin@poodll.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class manage_table extends \table_sql {
    ...
    /**
     * @var \enrol_plugin $poodllproviderplugin
     */
    protected $poodllproviderplugin;

    /**
     * @var bool $poodllproviderenabled
     */
    protected $poodllproviderenabled;
    ...
    public function __construct($courseid) {
        parent::__construct('enrol_poodllprovider_manage_table');
        ...
        $this->define_headers(array(
            get_string('name'),
            get_string('launchdetails', 'enrol_poodllprovider'),
            get_string('registrationurl', 'enrol_poodllprovider'),
            get_string('edit')
        ));
        ...
        $this->poodllproviderplugin = enrol_get_plugin('poodllprovider');
        $this->poodllproviderenabled = enrol_is_enabled('poodllprovider');
        $this->canconfig = has_capability('moodle/course:enrolconfig', \context_course::instance($courseid));
        $this->courseid = $courseid;

        // Set help icons.
        $launchicon = new \help_icon('launchdetails', 'enrol_poodllprovider');
        $regicon = new \help_icon('registrationurl', 'enrol_poodllprovider');
    ...
    public function col_launch($tool) {
        ...
        $cartridgeurllabel = get_string('cartridgeurl', 'enrol_poodllprovider');
        $cartridgeurl = $url;
        $secretlabel = get_string('secret', 'enrol_poodllprovider');
        $secret = $tool->secret;
        $launchurl = helper::get_launch_url($tool->id);
        $launchurllabel = get_string('launchurl', 'enrol_poodllprovider');
        ...
        $return = $OUTPUT->render_from_template("enrol_poodllprovider/copy_grid", $data);
    ...
    public function col_registration($tool) {
        ...
        $toolurllabel = get_string("registrationurl", "enrol_poodllprovider");
        $toolurl = $url;
    ...
    public function col_edit($tool) {
        ...
        $url = new \moodle_url('/enrol/poodllprovider/index.php', array('sesskey' => sesskey(), 'courseid' => $this->courseid));

        if ($this->poodllproviderplugin->can_delete_instance($instance)) {
        ...
        if ($this->poodllproviderenabled && $this->poodllproviderplugin->can_hide_show_instance($instance)) {
        ...
        if ($this->poodllproviderenabled && $this->canconfig) {
            $linkparams = array(
                'courseid' => $instance->courseid,
                'id' => $instance->id, 'type' => $instance->enrol,
                'returnurl' => new \moodle_url('/enrol/poodllprovider/index.php', array('courseid' => $this->courseid))
            );
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        $total = \enrol_poodllprovider\helper::count_lti_tools(array('courseid' => $this->courseid));
        $this->pagesize($pagesize, $total);
        $tools = \enrol_poodllprovider\helper::get_lti_tools(array('courseid' => $this->courseid), $this->get_page_start(),
            $this->get_page_size());
        $this->rawdata = $tools;
        // Set initial bars.
        if ($useinitialsbar) {
