 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_poodllprovider_helper_testcase extends advanced_testcase {
    ...
    public function test_update_user_profile_image() {
        ...
        \enrol_poodllprovider\helper::update_user_profile_image($this->user1->id, $this->getExternalTestFileUrl('/test.jpg'));
    ...
    public function test_enrol_user_max_enrolled() {
        ...
        // Set up the poodllprovider enrolment tool.
        $data = new stdClass();
        $data->maxenrolled = 1;
        $tool = $this->getDataGenerator()->create_lti_tool($data);

        // Now get all the information we need.
        $tool = \enrol_poodllprovider\helper::get_lti_tool($tool->id);

        // Enrol a user.
        $result = \enrol_poodllprovider\helper::enrol_user($tool, $this->user1->id);
        ...
        $result = \enrol_poodllprovider\helper::enrol_user($tool, $this->user2->id);

        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_poodllprovider\helper::ENROLMENT_MAX_ENROLLED, $result);
        $this->assertEquals(1, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_enrol_user_enrolment_not_started() {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_poodllprovider\helper::ENROLMENT_NOT_STARTED, $result);
    ...
    public function test_enrol_user_enrolment_finished() {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_poodllprovider\helper::ENROLMENT_FINISHED, $result);
        $this->assertEquals(0, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_count_lti_tools() {
        ...
        // Count all the tools.
        $count = \enrol_poodllprovider\helper::count_lti_tools();
        $this->assertEquals(4, $count);

        // Count all the tools in course 1.
        $count = \enrol_poodllprovider\helper::count_lti_tools(array('courseid' => $course1->id));
        $this->assertEquals(2, $count);

        // Count all the tools in course 2 that are disabled.
        $count = \enrol_poodllprovider\helper::count_lti_tools(array('courseid' => $course2->id, 'status' => ENROL_INSTANCE_DISABLED));
        $this->assertEquals(1, $count);

        // Count all the tools that are enabled.
        $count = \enrol_poodllprovider\helper::count_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    public function test_get_lti_tools() {
        ...
        // Get all the tools.
        $tools = \enrol_poodllprovider\helper::get_lti_tools();

        // Check that we got all the tools.
        $this->assertEquals(4, count($tools));

        // Get all the tools in course 1.
        $tools = \enrol_poodllprovider\helper::get_lti_tools(array('courseid' => $course1->id));
        ...
        $tools = \enrol_poodllprovider\helper::get_lti_tools(array('courseid' => $course2->id, 'status' => ENROL_INSTANCE_DISABLED));
        ...
        $tools = \enrol_poodllprovider\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    public function test_get_launch_url() {
        ...
        $id = $tool1->id;
        $launchurl = \enrol_poodllprovider\helper::get_launch_url($id);
        $this->assertEquals('https://www.example.com/moodle/enrol/poodllprovider/tool.php?id=' . $id, $launchurl->out());
    }
    ...
    public function test_get_cartridge_url() {
        ...
        $id = $tool1->id;
        $token = \enrol_poodllprovider\helper::generate_cartridge_token($id);
        $launchurl = \enrol_poodllprovider\helper::get_cartridge_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/poodllprovider/cartridge.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_poodllprovider\helper::get_cartridge_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/poodllprovider/cartridge.php/' . $id . '/' . $token . '/cartridge.xml',
                            $launchurl->out());
    ...
    public function test_get_proxy_url() {
        ...
        $id = $tool1->id;
        $token = \enrol_poodllprovider\helper::generate_proxy_token($id);
        $launchurl = \enrol_poodllprovider\helper::get_proxy_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/poodllprovider/proxy.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_poodllprovider\helper::get_proxy_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/poodllprovider/proxy.php/' . $id . '/' . $token . '/',
                            $launchurl->out());
    ...
    public function test_get_name() {
        ...
        $name = \enrol_poodllprovider\helper::get_name($tool1);
    ...
    public function test_get_description() {
        ...
        $description = \enrol_poodllprovider\helper::get_description($tool1);
        ...
        $tool2 = $generator->create_lti_tool($data);
        $description = \enrol_poodllprovider\helper::get_description($tool2);
    ...
    public function test_get_icon() {
        ...
        $icon = \enrol_poodllprovider\helper::get_icon($tool);
        $icon = $icon->out();
    ...
    public function test_verify_cartridge_token() {
        ...
        $token = \enrol_poodllprovider\helper::generate_cartridge_token($tool1->id);
        $this->assertTrue(\enrol_poodllprovider\helper::verify_cartridge_token($tool1->id, $token));
        $this->assertFalse(\enrol_poodllprovider\helper::verify_cartridge_token($tool1->id, 'incorrect token!'));
    }
    ...
    public function test_verify_proxy_token() {
        ...
        $token = \enrol_poodllprovider\helper::generate_proxy_token($tool1->id);
        $this->assertTrue(\enrol_poodllprovider\helper::verify_proxy_token($tool1->id, $token));
        $this->assertFalse(\enrol_poodllprovider\helper::verify_proxy_token($tool1->id, 'incorrect token!'));
    }
    ...
    public function test_set_xpath($parameters, $expected) {
        $helper = new ReflectionClass('enrol_poodllprovider\\helper');
    ...
    public function test_create_cartridge() {
        ...
        $cartridge = \enrol_poodllprovider\helper::create_cartridge($tool1->id);
        ...
        $this->assertContains("<blti:launch_url>$CFG->wwwroot/enrol/poodllprovider/tool.php?id=$tool1->id</blti:launch_url>", $cartridge);
    }
}
