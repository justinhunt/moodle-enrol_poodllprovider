...
/**
 * Handles synchronising grades for the enrolment poodllprovider.
 *
...
namespace enrol_poodllprovider\task;

/**
 * Task for synchronising grades for the enrolment poodllprovider.
 *
 * @package    enrol_poodllprovider
 * @copyright  2020 Justin Hunt <justin@poodll.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class sync_grades extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('tasksyncgrades', 'enrol_poodllprovider');
    }
    ...
    public function execute() {
        ...
        if (!enrol_is_enabled('poodllprovider')) {
            mtrace('Skipping task - ' . get_string('enrolisdisabled', 'enrol_poodllprovider'));
            return true;
        }

        // Get all the enabled tools.
        if ($tools = \enrol_poodllprovider\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED, 'gradesync' => 1))) {
            foreach ($tools as $tool) {
                ...
                if ($ltiusers = $DB->get_records('enrol_poodllprovider_users', array('toolid' => $tool->id), 'lastaccess DESC')) {
                    ...
                    foreach ($ltiusers as $ltiuser) {
                        ...
                        $body = \enrol_poodllprovider\helper::create_service_body($ltiuser->sourceid, $floatgrade);
