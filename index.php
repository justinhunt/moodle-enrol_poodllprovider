...
$pageurl = new moodle_url('/enrol/poodllprovider/index.php', array('courseid' => $courseid));
...
if ($action) {
    if ($action === 'delete') {
        if ($poodllproviderplugin->can_delete_instance($instance)) {
            if ($confirm) {
                $poodllproviderplugin->delete_instance($instance);
                redirect($PAGE->url);
            }

            $yesurl = new moodle_url('/enrol/poodllprovider/index.php',
            ...
            $displayname = $poodllproviderplugin->get_instance_name($instance);
        ...
        if ($poodllproviderplugin->can_hide_show_instance($instance)) {
            if ($instance->status != ENROL_INSTANCE_DISABLED) {
                $poodllproviderplugin->update_status($instance, ENROL_INSTANCE_DISABLED);
                redirect($PAGE->url);
            }
        }
        ...
        if ($poodllproviderplugin->can_hide_show_instance($instance)) {
            if ($instance->status != ENROL_INSTANCE_ENABLED) {
                $poodllproviderplugin->update_status($instance, ENROL_INSTANCE_ENABLED);
                redirect($PAGE->url);
            }
        }
    }
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('toolsprovided', 'enrol_poodllprovider'));
echo "<p>" .get_string('toolsprovided_help', 'enrol_poodllprovider') . "</p>";
...
if (\enrol_poodllprovider\helper::count_lti_tools(array('courseid' => $courseid)) > 0) {
    ...
    $notify = new \core\output\notification(get_string('notoolsprovided', 'enrol_poodllprovider'),
...
if ($poodllproviderplugin->can_add_instance($course->id)) {
    echo $OUTPUT->single_button(new moodle_url('/enrol/editinstance.php',
        array(
            'type' => 'poodllprovider',
            'courseid' => $course->id,
            'returnurl' => new moodle_url('/enrol/poodllprovider/index.php', array('courseid' => $course->id)))
        ),
        get_string('add'));
}
